...
mui.init({
    swipeBack:true,
    pullRefresh: {
        container: '#delivery_movelist_pullrefresh',
        up: {
            auto: false,
            contentrefresh: '正在加载...',
...
function pulluprefresh(){
    mui.later(function(){
        var pullrefresh = document.body.querySelector('#delivery_movelist_pullrefresh');
        ...
        mui.ajax('/aaswechat/wms/deliverymovemore',{
            ...
            success:function(data){
                ...
                if (dresult.movecount==0){
                    mui('#delivery_movelist_pullrefresh').pullRefresh().endPullupToRefresh(true);
                    return ;
                }else if(dresult.movecount<20){
                    mui('#delivery_movelist_pullrefresh').pullRefresh().endPullupToRefresh(true);
                }else{
                    mui('#delivery_movelist_pullrefresh').pullRefresh().endPullupToRefresh(false);
                }
...
mui.ready(function(){
    ...
    mui(".mui-content").on("tap", "li.aas-move", function(){
        ...
        mui.openWindow({'url': '/aaswechat/wms/deliverymovedetail/'+moveid, 'id': 'deliverymovedetail'});
    });
